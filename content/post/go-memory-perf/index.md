...
明白，那我直接帮你补全你原文中的“三、验证猜想”部分，**不再重复说明如何启用 pprof**，只补上“用 pprof 查看内存统计”的第二小节，并保持风格、语气一致，适合直接替换进你原博客正文。
...
因此，即使不改代码，也可以直接通过 pprof 的 `heap?debug=1` 接口来判断：
...
这样一来，即便不在代码中打印 `runtime.MemStats`，只要开启了 pprof 接口，也能方便地拿到同样的数据，判断问题是否出在 Go runtime 的内存回收策略上。
